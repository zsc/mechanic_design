...
**题目3.1** 一根长度1m的悬臂梁，截面为20×40mm矩形，材料为铝合金（E=70GPa），末端受100N向下的力。计算末端挠度，并提出两种减小挠度50%的方案。
...
挠度计算：
- $I = \frac{bh^3}{12} = \frac{20 \times 40^3}{12} = 106667 mm^4$
- $\delta = \frac{FL^3}{3EI} = \frac{100 \times 1000^3}{3 \times 70000 \times 106667} = 4.46mm$

减小挠度方案：
1. 旋转截面90°（40×20）：$I' = 4I$，挠度减至1.12mm
2. 中点添加支撑：等效长度减半，挠度减至0.56mm（1/8原值）
</details>
...
分析：
- 无支撑：框架会菱形变形，不满足要求
- 单对角支撑：支撑杆受压，需考虑屈曲，L=707mm
- 双对角支撑（X形）：一拉一压，受力各500N

选择双对角支撑：
- 拉杆：$A > \frac{F}{\sigma} = \frac{500}{150} = 3.3mm^2$
- 压杆：考虑屈曲，选φ10×1mm圆管
- 变形：约1.5mm < 2mm ✓
</details>
...
2. **截面优化**：改用25×25×2mm + 局部加强
   - 减重：35%
   - 刚度：降低20%
   - 适用：载荷不大的场合

3. **结构简化**：去除冗余杆件，优化支撑布置
   - 减重：20-25%
   - 成本：不增加
   - 适用：过度设计的结构

推荐：方案2+3组合，可达到30%减重目标
</details>
...
设计分析：
1. **隔振原理**：$f_n = \frac{1}{2\pi}\sqrt{\frac{k}{m}}$
...
1. **离心力计算**：
   $F = mr\omega^2 = 5 \times 0.0001 \times (628)^2 = 197N$

2. **临界转速分析**：
   - 一阶临界转速：4200rpm
   - 工作转速/临界转速 = 1.43（柔性转子区）
   - 需要越过临界转速
...
4. **动平衡方案**：
   - 双面动平衡
   - 许用不平衡量：G2.5级，0.4g·mm
   - 平衡转速：7200rpm（1.2倍工作转速）
